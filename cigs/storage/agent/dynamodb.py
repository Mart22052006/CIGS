...
class DynamoDbAgentStorage(AgentStorage):
    ...
    def fetch_user_from_db(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Получение пользователя по user_id из DynamoDB."""
        try:
            response = self.table.query(
                IndexName="user_id-index",
                KeyConditionExpression=Key("user_id").eq(user_id),
                ProjectionExpression="user_id, user_data"
            )
            items = response.get("Items", [])
            if not items:
                return None

            user_data = items[0].get("user_data", {})
            return {
                "id_user": user_data.get("id_user", user_id),
