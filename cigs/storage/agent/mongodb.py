...
class MongoAgentStorage(AgentStorage):
    ...
    async def fetch_user_from_db(user_id: str) -> Optional[UserSchema]:
        """Получение пользователя из MongoDB по user_id."""
        try:
            storage = MongoAgentStorage(collection_name="users")  # Используем коллекцию пользователей
            user_doc = storage.collection.find_one({"user_id": user_id})

            if not user_doc:
                return None

            return UserSchema(
                id_user=user_doc["user_id"],
