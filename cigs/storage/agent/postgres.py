...
class PgAgentStorage(AgentStorage):
    ...
    async def fetch_user_from_db(user_id: str) -> Optional[UserSchema]:
        """Получение пользователя из базы данных PostgreSQL через SQLAlchemy"""
        try:
            async with PgAgentStorage.Session() as session:
                result = await session.execute(
                    select(PgAgentStorage.table).where(PgAgentStorage.table.c.user_id == user_id)
                )
