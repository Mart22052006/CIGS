...
from cigs.storage.agent.base import AgentStorage
...
class SqlAgentStorage(AgentStorage):
    ...
    from cigs.agent import AgentSession
    ...
    async def fetch_user_from_db(user_id: str) -> Optional[UserSchema]:
        """Получение пользователя из таблицы users по ID через SQLAlchemy"""
        try:
            async with SqlAgentStorage.Session() as session:
                result = await session.execute(select(SqlAgentStorage.table).where(SqlAgentStorage.table.c.user_id == user_id))
