...
from cigs.api.workspace import log_workspace_event
from cigs.api.schemas.workspace import (
    WorkspaceSchema,
    WorkspaceCreate,
    WorkspaceUpdate,
    WorkspaceEvent,
)
from cigs.api.schemas.team import TeamSchema, TeamIdentifier
from cigs.cli.config import PhiCliConfig
from cigs.cli.console import (
    console,
    print_heading,
    print_info,
    print_subheading,
...
from cigs.infra.type import InfraType
from cigs.infra.resources import InfraResources
from cigs.workspace.config import WorkspaceConfig
from cigs.workspace.enums import WorkspaceStarterTemplate
from cigs.utils.common import str_to_int
...
def create_workspace(
    ...
    from cigs.cli.operator import initialize_phi
    from cigs.utils.filesystem import rmdir_recursive
    from cigs.workspace.helpers import get_workspace_dir_path
    from cigs.utils.git import GitCloneProgress
...
def setup_workspace(ws_root_path: Path) -> Optional[WorkspaceConfig]:
    ...
    from cigs.cli.operator import initialize_phi
    from cigs.utils.git import get_remote_origin_for_dir
    from cigs.workspace.helpers import get_workspace_dir_path
    ...
    if phi_config.user is None:
        from cigs.api.user import create_anon_user
    ...
    if phi_config.user is not None:
        ...
        if ws_schema is None or ws_schema.id_workspace is None:
            from cigs.api.team import get_teams_for_user
            from cigs.api.workspace import create_workspace_for_user
        ...
        if git_remote_origin_url is not None and ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from cigs.api.workspace import update_workspace_for_user, update_workspace_for_team
