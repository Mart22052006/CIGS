...
from cigs.api.api import api, invalid_response
from cigs.api.routes import ApiRoutes
from cigs.api.schemas.user import UserSchema, EmailPasswordAuthSchema, UserRole
...
def authenticate_and_get_user(auth_token: str, existing_user: Optional[UserSchema] = None) -> Optional[UserSchema]:
    ...
    from cigs.cli.credentials import read_auth_token
    ...
    with api.Client() as api_client:
        try:
            ...
            return UserSchema(
                ...
                role=UserRole(user_data.get("role", "user")),  # По умолчанию все - пользователи
                username=user_data.get("username"),
                is_machine=user_data.get("is_machine", False),
            )
